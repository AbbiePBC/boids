...
pub(crate) fn maybe_reflect_off_boundaries(mut boid_to_update: Boid, frame_width: f32, frame_height: f32, time_per_frame: f32) -> Boid {
    ...
    if boid_to_update.x_pos >= (frame_width - 5.0) ||
        boid_to_update.x_pos <= 5.0 {
        boid_to_update.x_vel = -boid_to_update.x_vel;
    }
    if boid_to_update.y_pos >= (frame_height - 5.0) ||
        boid_to_update.y_pos <= 5.0 {
...
impl Boid {
    ...
    pub(crate) fn is_crowded_by_boid(&self, other_boid: &Boid, max_dist_before_boid_is_no_longer_crowded: f32) -> bool {
    ...
    pub(crate) fn is_within_sight_of_local_boid(&self, other_boid: &Boid, max_dist_of_local_boid: f32) -> bool {
        return (self.x_pos - other_boid.x_pos).abs() < max_dist_of_local_boid &&
            (self.y_pos - other_boid.y_pos).abs() < max_dist_of_local_boid;
    }

    pub(crate) fn align_boid(&self,
...
mod tests {
    ...
    fn test_boundary_reflected_when_velocity_is_zero(){
        ...
        let updated_boid = maybe_reflect_off_boundaries(boid_to_update, max_x_position, max_y_position, 1.0);
